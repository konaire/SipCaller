...
Для того, чтобы зарегистрироваться в SIP на андроид нужно сначала взять инстанс от `SipManager`, здесь же следует сразу проверить поддерживает ли устройство апи для sip, а затем уже надо инициализировать sip профиль, там тоже есть пара острых моментов.
...
`CallEvent` - это простейший класс данных, который содержит одно поле для `Intent`. Кроме этого в `DataReceiver` была добавлена передача статусов о звонке между сервисом и активити и наоборот.
